...
//========================
  ...
  //-- Optimized range by man.. when rebinning 20  
	//cout << "sig :" << h_sig->Integral(15,17) << endl;
	//cout << "bkg :" << h_bgr->Integral(15,17) << endl;
	//cout << "dat :" << h_data->Integral(15,17) << endl;
	//--results	
		//sig :6.28504
		//bkg :7.41814
		//dat :16

  //-- Optimized range by man.. when no rebinning  
	//cout << "sig :" << h_sig->Integral(295,339) << endl;
	//cout << "bkg :" << h_bgr->Integral(295,339) << endl;
	//cout << "dat :" << h_data->Integral(295,339) << endl;
	//cout << h_sig->GetXaxis()->FindBin(125) << endl;  // Bin number of 125 GeV
	//--results
		//sig :5.77787
		//bkg :5.74414
		//dat :15
 ...
 //-- Count Total bin numner
cout <<	"sigNbin: " << h_sig->GetNbinsX() << endl;
//cout << "bkgNbin: " << h_bgr->GetNbinsX() << endl;
//cout << "datNbin: " << h_data->GetNbinsX() << endl;
...
//-- Prepare cumulative histogram for signal + background 
  ...
  for (int i_bin = 1; i_bin < h_bgr->GetNbinsX(); i_bin++){
       ...
       //printf("  REBINNED HISTOGRAM:  bin %d, Ndata = %d\n",i_bin,(int)h_data->GetBinContent(i_bin));
		//cout << i_bin <<" " <<h_sig->GetBinContent(i_bin) << endl; 
  }



//cout << "sig :" << sigsum << endl;
//cout << "bkg :" << bkgsum << endl;
//cout << "dat :" << datsum << endl;
...
//=============================================================
  ...
  //---------------------------------
	...
	//-- Make a mass window range
	double k= masswindow_fullwidth/2.;
	double start_bin = h_sig->GetXaxis()->FindBin(125-k);
	double end_bin	 = h_sig->GetXaxis()->FindBin(125+k);
	
		
	//-- Integral mass window amd calculate the number of expected evets
	double N_exptect_sig = h_sig->Integral(start_bin,end_bin);
	double N_exptect_bgr = h_bgr->Integral(start_bin,end_bin);
	double N_exptect_dat = h_data->Integral(start_bin,end_bin);


		//-- Calculate Expected and observed significance
	double pvalue_expected       = IntegratePoissonFromRight(N_exptect_bgr,N_exptect_bgr + N_exptect_sig);
    double significance_expected = ROOT::Math::gaussian_quantile_c(pvalue_expected,1);
    
	if(i_bin <= 3){ h_masswindow_expected->SetBinContent(i_bin,0);	 //-- Too low number of events
	}else{
	h_masswindow_expected->SetBinContent(i_bin, significance_expected);
	}
    
		//-- [c] compute OBSERVED significance and save in histogram
    double pvalue_observed       = IntegratePoissonFromRight(N_exptect_bgr,N_exptect_dat);
	...
	//-- Check results
	//cout << "range factor: " << k << endl;
	//cout << "sig: " << N_exptect_sig << endl;
	//cout << "bgr: " << N_exptect_bgr << endl;
	//cout << "dat: " << N_exptect_dat << endl;
	//cout << "expected significance: " << significance_expected << endl;
	//cout << "observed significance: " << significance_observed << endl;
  ...
  //-- Find the optimized range	
	int binamax_exp = h_masswindow_expected->GetMaximumBin();
	int binamax_obs = h_masswindow_observed->GetMaximumBin();
	double xmax_exp = h_masswindow_expected->GetXaxis()->GetBinCenter(binamax_exp);
	double xmax_obs = h_masswindow_observed->GetXaxis()->GetBinCenter(binamax_obs);
	cout << "Optimized expected significance: " << 125-xmax_exp << " ~ "<< 125+xmax_exp << " GeV" << endl;
	cout << "Optimized observed significance: " << 125-xmax_obs << " ~ "<< 125+xmax_obs << " GeV" << endl;
	cout << "Expected Significance: "<< h_masswindow_expected->GetBinContent(binamax_exp) << " sigma" << endl;
  ...
  TLine *l = new TLine(0,5,25,5); // Point the discovery level ( 5 sigma )
  l->SetLineColor(2);
  l->SetLineWidth(2);
