...
// --Event Loop start
	...
	// --Select Final state leptons
		...
		if(eleSelTCA->GetEntries()+muSelTCA->GetEntries() <4 ) continue;
		num_lepcut1++;
		num_lepcut2++;
	...
	// --4 Electrons	
		if(eleSelTCA->GetEntries() == 4){
			...
			if(elePtr1->Charge + elePtr2->Charge !=0){
			num_C1324ele++;

				TLorentzVector M13Vec = ele1Vec+ele3Vec;
				TLorentzVector M24Vec = ele2Vec+ele4Vec;
				TLorentzVector M13Z1Vec;
				TLorentzVector M13Z2Vec;
				TLorentzVector M24Z1Vec;
				TLorentzVector M24Z2Vec;
					
				if(M13Vec.M() > 50 && ele1Vec.Pt() > 20 && ele3Vec.Pt() >10){
					M13Z1Vec = M13Vec;
				}else{
					M13Z2Vec = M13Vec;
				}
				
				if(M24Vec.M() > 50 && ele2Vec.Pt() > 20 && ele4Vec.Pt() >10){
					 M24Z1Vec = M24Vec;
				}else{
					 M24Z2Vec = M24Vec;
				}
				...
				TLorentzVector Z1Vec = M13Z1Vec+M24Z1Vec;
				TLorentzVector Z2Vec = M13Z2Vec+M24Z2Vec;
	...
	// --4 Muon
		if(muSelTCA->GetEntries() == 4){
			...
			if(muPtr1->Charge + muPtr2->Charge !=0){
			num_C1324mu++;
				...
				if(M13Vec.M() > 50 && mu1Vec.Pt() > 20 && mu3Vec.Pt() >10){
					M13Z1Vec = M13Vec;
				}else{
					M13Z2Vec = M13Vec;
				}
				
				if(M24Vec.M() > 50 && mu2Vec.Pt() > 20 && mu4Vec.Pt() >10){
					 M24Z1Vec = M24Vec;
				}else{
					 M24Z2Vec = M24Vec;
				}
...
*/
	...
	cout << " ##########    num_lepcut1" << num_lepcut1 << endl;
	cout << " ##########    num_lepcut2" << num_lepcut2 << endl;
	cout << " ########## 	Totevt_cut_base" << Totevt_cut_base << endl;
	cout << " ##########	num_4ele  " 		<<	num_4ele   << endl;
	cout << " ##########	ele_C1324 " 		<<	num_C1324ele   << endl;
	cout << " ##########	ele_C1324_withcut " <<	num_C1324ele_withcut   << endl;
	cout << " ##########	ele_C1234 " 		<<	num_C1234ele   << endl;
	cout << " ##########	ele_C1234_withcut " <<	num_C1234ele_withcut   << endl;
